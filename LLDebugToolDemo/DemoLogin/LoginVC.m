...
- (void)viewDidLoad {
    ...
    _account.placeholder=[NSString stringWithFormat:@"账号请输入Email"];
    _account.delegate = self ;
    [self.view addSubview:_account];
    
    _password=[[UITextField alloc] initWithFrame:CGRectMake(20, 260, self.view.frame.size.width-40, 50)];
    _password.backgroundColor=[UIColor whiteColor];
    _password.placeholder=[NSString stringWithFormat:@"密码请输入Password"];
...
-(void)pressBtn:(UIButton*)btn{
    ...
    if([_account.text isEqual:@"Email"] && [_password.text isEqual:@"Password"]){
