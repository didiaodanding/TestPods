...
/*
 ...
 LLFMDatabaseQueue is released on another thread and we're in the middle of doing
 something in dispatch_sync
 
 */

/*
 * A key used to associate the LLFMDatabaseQueue object with the dispatch_queue_t it uses.
 * This in turn is used for deadlock detection by seeing if inDatabase: is called on
 * the queue's dispatch queue, which should not happen and causes a deadlock.
 */
static const void * const kDispatchQueueSpecificKey = &kDispatchQueueSpecificKey;

@interface LLFMDatabaseQueue () {
    dispatch_queue_t    _queue;
    LLFMDatabase          *_db;
}
@end

@implementation LLFMDatabaseQueue

+ (instancetype)databaseQueueWithPath:(NSString *)aPath {
    LLFMDatabaseQueue *q = [[self alloc] initWithPath:aPath];
...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath flags:(int)openFlags {
    LLFMDatabaseQueue *q = [[self alloc] initWithPath:aPath flags:openFlags];
...
#endif
        if (!success) {
            NSLog(@"LLFMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(__attribute__((noescape)) void (^)(LLFMDatabase *db))block {
    ...
    LLFMDatabaseQueue *currentSyncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
    ...
    dispatch_sync(_queue, ^() {
        ...
        LLFMDatabase *db = [self database];
        
        block(db);
        
        if ([db hasOpenResultSets]) {
            NSLog(@"Warning: there is at least one open result set around after performing [LLFMDatabaseQueue inDatabase:]");
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(LLFMDatabase *db, BOOL *rollback))block {
    FMDBRetain(self);
    dispatch_sync(_queue, ^() { 
...
- (void)inImmediateTransaction:(__attribute__((noescape)) void (^)(LLFMDatabase * _Nonnull, BOOL * _Nonnull))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(LLFMDatabase *db, BOOL *rollback))block {
