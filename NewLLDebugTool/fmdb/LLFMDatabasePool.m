...
- (void)pushDatabaseBackInPool:(LLFMDatabase*)db;
- (LLFMDatabase*)db;

@end


@implementation LLFMDatabasePool
...
- (void)pushDatabaseBackInPool:(LLFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The LLFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
