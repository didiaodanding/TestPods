...
- (LLFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark LLFMDatabase instantiation and deallocation
...
// if ([LLFMDatabase FMDBVersion] >= 0x0240) { … }
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    LLFMDatabase *self = (__bridge LLFMDatabase*)f;
...
// but for folks who don't bother noticing that the interface to LLFMDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        LLFMResultSet *rs = (LLFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(LLFMResultSet *)resultSet {
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    ...
    if (!query) {
        NSLog(@"API misuse, -[LLFMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)goodConnection {
    ...
    LLFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The LLFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The LLFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The LLFMDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The LLFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"LLFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (LLFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (LLFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    rs = [LLFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (LLFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (LLFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (LLFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (LLFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    LLFMResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (LLFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(BOOL *rollback))block {
    ...
    NSString *errorMessage = NSLocalizedStringFromTable(@"Save point functions require SQLite 3.7", @"FMDB", nil);
    if (self.logsErrors) NSLog(@"%@", errorMessage);
    return [NSError errorWithDomain:@"LLFMDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
