...
+ (NSString *)bs_backtrace:(uintptr_t *)backtraceBuffer backtraceLength:(int)backtraceLength{
    return _bs_backtrace(backtraceBuffer,backtraceLength) ;
}
...
+ (BOOL)bs_isStackOverflow:(thread_t)thread {
    return _bs_isStackOverflow(thread) ;
}

#define KSSC_STACK_OVERFLOW_THRESHOLD 150
BOOL _bs_isStackOverflow(thread_t thread){
    int i = 0;
    ...
    for(; i < KSSC_STACK_OVERFLOW_THRESHOLD; i++) {
    ...
    if(i >= KSSC_STACK_OVERFLOW_THRESHOLD)
    {
        return true ;
    }else{
        return false ;
...
NSString *_bs_backtrace(uintptr_t *backtraceBuffer,int backtraceLength){
    ...
    for (int i = 1; i < backtraceLength; ++i) {
