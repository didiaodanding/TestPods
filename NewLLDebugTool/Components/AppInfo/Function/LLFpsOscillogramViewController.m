//
//  LLFpsOscillogramViewController.m
//  LLDebugToolDemo
//
...
@interface LLFpsOscillogramViewController ()

@end

@implementation LLFpsOscillogramViewController
...
- (void)closeBtnClick{
    [[LLDebugTool sharedTool] saveFpsPerformanceCurveSwitch:NO] ;
...
- (void)didReceiveLLAppHelperDidUpdateAppInfosNotification:(NSNotification *)notifi {
    ...
    long fps = (long)[userInfo[LLAppHelperFPSKey] floatValue];
    
    // 0~60  对应 高度0~_self.oscillogramView.frame.size.height
    NSLog(@"haleli >>> fps : %ld",fps) ;
