...
- (void)forwardInvocation:(NSInvocation *)invocation
{
    ...
    if ([NSStringFromSelector(invocation.selector) isEqualToString:@"webViewDidFinishLoad:"]) {
        ...
        if (@available(iOS 10.0, *)) {
            timingStr = [webView stringByEvaluatingJavaScriptFromString:@"JSON.stringify(window.performance.timing.toJSON())"];
            ...
            timingStr = [webView stringByEvaluatingJavaScriptFromString:@"JSON.stringify(flatten(window.performance.timing))"];
        }
        
        NSString *responseStr = [webView stringByEvaluatingJavaScriptFromString:@"document.documentElement.outerHTML.toString()"];
        ...
        model.url = webView.request.URL;
        model.method = webView.request.HTTPMethod;
        model.headerFields = [webView.request.allHTTPHeaderFields mutableCopy];
        if (webView.request.HTTPBody) {
            model.requestBody = [LLTool convertJSONStringFromData:webView.request.HTTPBody];
        } else if (webView.request.HTTPBodyStream) {
            NSData* data = [self dataFromInputStream:webView.request.HTTPBodyStream];
